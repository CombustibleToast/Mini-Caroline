...
async function getQuery(query) {
	...
	const pushVideoInfo = async (url) => {
		...
		try{
			...
			loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] ytdl.getInfo() error:\n${e.message}`);
		}
		if(videoInfo)
	...
	//query is not a link, seqrch
		...
		try {
			...
			loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] ytsr for "${query}" encountered an error:\n${e.stack}`);
		}
	}
