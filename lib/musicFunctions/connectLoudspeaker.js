...
module.exports = {
	...
	async execute(interaction) {
		...
		if (loudspeakerAssignmentResult.status == "new assignment") {
			...
			const loudspeakerClient = interaction.client;
	...
	*/
	...
	loudspeakerClient.player.on(AudioPlayerStatus.Idle, () => {
		...
		if(loudspeakerClient.player.songErroredOut){
			...
			loudspeakerClient.functions.get("musicLog").execute(`Song errored`)
			loudspeakerClient.functions.get("startNextSong").execute(loudspeakerClient);
		}
		else
	...
	loudspeakerClient.player.on('error', error => {
		...
		loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] ${loudspeakerClient.user.username} encountered an error:\n${error.stack}`);
	});
	loudspeakerClient.player.on('stateChange', (oldState, newState) => {
		loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [INFO] ${loudspeakerClient.user.username}: ${oldState.status} -> ${newState.status}`);
	});
	loudspeakerClient.connection.on('error', error => {
		loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] ${loudspeakerClient.user.username} connection error:\n${error.message}`);
	});
}
...
function autoplayNextSong(loudspeakerClient) {
	...
	loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [INFO] ${loudspeakerClient.user.username} autoplaying next song in queue.`);
	...
	if (loudspeakerClient.queue.length != 0)
		loudspeakerClient.functions.get("startNextSong").execute(loudspeakerClient); //using interaction here might lead to some problems
}