...
module.exports = {
    ...
    async execute(loudspeakerClient) {
        ...
        if (loudspeakerClient.player.songErroredOut) {
            ...
            loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] Song errored out! Attempting to start it again from ${options.begin} (${startDate.getMilliseconds()})`);
        }
        else {
            //save the timestamp in case an error occurs
            //Don't save the time stamp if continuing from an errored state
            loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [INFO] Playing new song. Timestamp: ${Date.now()}`);
