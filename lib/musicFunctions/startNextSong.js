...
module.exports = {
    ...
    async execute(loudspeakerClient) {
        ...
        if (loudspeakerClient.player.songErroredOut) {
            ...
            loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [WARN] Song errored out, downloading song from ${options.begin} (${startDate.getMilliseconds()})`);
        }
        else {
            //save the timestamp in case an error occurs
            //Don't save the time stamp if continuing from an errored state
            loudspeakerClient.functions.get("musicLog").execute(`MUSIC - [INFO] Song started normally; collected timestamp ${Date.now()}`);
